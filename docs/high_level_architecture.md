...
The synthetic gateway now signs a client-held task-state envelope bound to the authenticated user, gateway audience, requested tool, and canonical parameters. It validates that envelope on every stateless request, requires `Mcp-Method` and `Mcp-Name` to exactly match the body routing fields, and rejects `_meta` fields that attempt to supply identity, audience, or scope. This is a local contract test, not implementation of the 2026-07-28 transport or Tasks extension.
