...
The local task contract also enforces a per-user active-task budget and supports cancellation while a task is pending. A cancelled state is rejected before tool execution, even if its signature is still valid. Phase 27 adds store-backed worker ownership: `run_task` requires a worker identity, claims a task with an expiring lease, and only the lease owner may publish a result. A lease race allows one claimant, while lease expiry is quarantined as `recovery_required` rather than retried. This remains a synthetic local contract: it has no heartbeat renewal, distributed queue, cancellation of already-running work, resource accounting, or protected multi-node lease authority.
...
The task store can now be backed by local SQLite. Completed task results remain available after gateway restart. On startup, any persisted `in_progress` task becomes `recovery_required`; Nova Aegis returns `REVIEW` and does not automatically replay the handler. Phase 28 adds renewable worker leases with monotonic fencing tokens: only the current worker and fence may renew or publish a result, and stale workers cannot commit after a newer claim. Recovery requires a future authorized reconciliation workflow with an independent execution receipt.

The current synthetic reconciliation contract requires the original authenticated task owner to hold the operator role and an explicit recovery scope. Phase 29 additionally requires a distinct operator to approve the exact resolution, external receipt, and result hash; the owner then performs reconciliation, and the approval is consumed only after receipt verification succeeds. Phase 31 adds an injectable approval store with SQLite persistence and atomic single-use consumption, so a valid approval survives gateway restart without becoming reusable. Phase 32 adds durable revocation and makes approval consumption precede task finalization: if task finalization fails, the approval is already spent and the task remains `recovery_required`, never a reconciled state with reusable authority. Phase 33 adds a durable recovery journal created in the same local approval-store transaction as approval consumption. Phase 34 binds each journal payload to a deterministic integrity digest; startup verifies that digest before replay and audits tampered entries as blocked. Phase 36 adds an opt-in `SQLiteRecoveryStore` that co-locates task, approval, and journal state and commits approval consumption, journal creation, task finalization, and journal completion in one local SQLite transaction. Phase 37 optionally authenticates those journal records with a configured HMAC key; startup verifies through the store boundary and blocks replay on key mismatch or forged payloads. Phase 38 persists a journal `key_id` and supports trusted-key overlap, active-key rotation, and old-key retirement; unknown or retired keys block replay. Phase 39 directly probes concurrent finalization and proves that only one contender can consume approval and commit task recovery state. Startup replays only verified journal bookkeeping, never the external handler, and clears the journal only after the task reaches the recorded reconciled state. This closes the synthetic restart crash window and the local cross-store commit gap while preserving fail-closed behavior for missing, mismatched, tampered, unauthenticated, or contended tasks. It accepts only `completed` or `abandoned` resolution with a non-empty external receipt reference, stores the reconciliation result, and never invokes the handler as part of resolution. Phase 26 adds an injectable external receipt verifier: the local implementation signs receipts and binds them to task identity, tool, owner, gateway audience, canonical parameters, resolution, and result hash.

Phase 41 moves journal key lookup and lifecycle operations behind an injectable `JournalKeyProvider`. The default provider is local and synthetic; lifecycle changes require explicit synthetic authority, unknown keys fail closed, and provider failures propagate so recovery transactions roll back. This is a governance boundary for testing, not protected key custody, a vault, organizational identity, or a production rotation service. Phase 43 adds an optional evidence hierarchy, applies explicit authority and hierarchy scopes before lexical ranking, and records candidate, filtering, score, and selection details in the retrieval audit event. Phase 52 adds deterministic trace serialization and replay; Phase 53 binds each trace to canonical corpus and trace digests; Phase 54 exposes only hash-chain-verified retrieval traces through the SQLite audit boundary; Phase 56 adds a versioned synthetic HMAC corpus manifest; Phase 57 tests replay through concurrent independent local SQLite connections; Phase 58 adds an opt-in provenance gate for reliability history; and Phase 59 adds synthetic receipt revocation and conflict rejection. These controls improve reconstruction and failure behavior in the tested local path but do not make memory authoritative, authenticate external hierarchy metadata, restore historical sources, establish distributed durability, independently witness external effects, or enable networked MCP.

The receipt registry is still a local proof, not a production external evidence service. It does not yet query a real tool, use public-key signatures, preserve receipt revocation, resolve conflicting receipts, or provide an independently durable receipt ledger.
