...
Phase 41 moves journal key lookup and lifecycle operations behind an injectable `JournalKeyProvider`. The default provider is local and synthetic; lifecycle changes require explicit synthetic authority, unknown keys fail closed, and provider failures propagate so recovery transactions roll back. This is a governance boundary for testing, not protected key custody, a vault, organizational identity, or a production rotation service. Phase 43 adds an optional evidence hierarchy, applies explicit authority and hierarchy scopes before lexical ranking, and records candidate, filtering, score, and selection details in the retrieval audit event. These traces improve reconstruction of the tested local retrieval path but do not make memory authoritative or establish authenticated external hierarchy metadata.
